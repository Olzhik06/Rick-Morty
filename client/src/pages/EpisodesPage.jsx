...
export default function EpisodesPage() {
  ...
  async function load() {
    ...
    try {
      const res = await apiGet(`/api/episodes?${qs}`);
      setData(res);
    } catch (e) {
      if (e?.status === 404) {
  ...
  return (
    <div className="card" style={{ padding: 16 }}>
      ...
      {loading && <SkeletonGrid count={10} card="episode" />}
            ...
            {data.results.map((ep) => (
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
                  <div style={{ fontWeight: 900, letterSpacing: "-0.01em" }}>{ep.name}</div>
                  <Badge>{ep.episode}</Badge>
                </div>

                <div style={{ marginTop: 8, color: "var(--muted)", fontSize: 13 }}>
                  Air date: <b>{ep.air_date}</b>
                </div>

                <div style={{ marginTop: 12, display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <Badge>Characters: {ep.characters?.length ?? 0}</Badge>
                </div>
              </Link>
            ))}
          </div>
